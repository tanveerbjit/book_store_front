...
function CategoriesDropdown() {
  ...
  return (
    <div className="col-lg-3 d-none d-lg-block" ref={dropdownRef}>
      <div className={`btn d-flex align-items-center justify-content-between ${isCollapsed ? 'bg-primary' : 'bg-primary'} w-100`} onClick={toggleCollapse} style={{ height: '65px', padding: '0 30px', cursor: 'pointer' }}>
        <h6 className={`text-${isCollapsed ? 'dark' : 'dark'} m-0`}>
          <i className={`fa fa-${isCollapsed ? 'angle-down' : 'angle-up'} mr-2`}></i>Categories
        </h6>
      </div>
      {isCollapsed && (
        <nav className={`position-absolute navbar navbar-vertical navbar-light align-items-start p-0 bg-light`} style={{ width: 'calc(100% - 30px)', zIndex: '999' }}>
          <div className="navbar-nav w-100 border border-4 border-black">
            <div className="nav-item dropdown dropright">
              {
                book.filter_data.category.map((element,index)=><List payload={element.payload} link={element.link}/>)
              }
            </div>
          </div>
        </nav>
